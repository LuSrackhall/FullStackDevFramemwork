...
func InitLogger(log_dir_path string) {

	// 检查路径是否存在, 如果不存在则创建路径
	if _, err := os.Stat(log_dir_path); os.IsNotExist(err) {
		// 如果路径不存在，创建路径
		err := os.MkdirAll(log_dir_path, os.ModePerm)
		if err != nil {
			slog.Error("日志目录路径'"+log_dir_path+"'创建时出错。", "err", err.Error())
		} else {
			slog.Info("日志目录路径创建成功。", "日志文件存储目录的路径为", log_dir_path)
		}
	} else if err != nil {
		slog.Error("检查日志目录路径'"+log_dir_path+"'时出错。", "err", err.Error())
	} else {
		slog.Info("日志目录路径已存在且无异常。", "日志文件存储目录的路径为", log_dir_path)
	}

	// 确保正在写入的日志文件名为 log.jsonl
	log_file_pathAndName := filepath.Join(log_dir_path, "log.jsonl")
	fmt.Println("日志文件的完整路径为:", log_file_pathAndName)
